...
cShaderProgram* cShaderManager::getShaderProgPointerFromID(unsigned int shaderID)
{
...
cShaderProgram* cShaderManager::getShaderProgPointerFromFriendlyName(std::string friendlyName)
{
...
void cShaderProgram::loadUniforms(void)
{
...
bool cShaderProgram::m_Assign2DTextureToFirstEmptySlot(sTextureBindBlendInfo *pTexBlendInfo)
{
	unsigned int bindIndex = 0;
	unsigned int numberOfTextureSlots = (unsigned int)this->vec2DTexBindings.size();
	for (; bindIndex != numberOfTextureSlots; bindIndex++)
